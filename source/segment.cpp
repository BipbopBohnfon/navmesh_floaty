...
namespace NavMesh {
	...
	bool Segment::Intersects(const Point& a1, const Point& a2) const
	{
		...
		if ((v1 ^ v2) * (v1 ^ v3) > EPSILON) return false;

		Point u1 = a2 - a1;
		Point u2 = b - a1;
		Point u3 = e - a1;
		// collinear vectors here mean no intersection. The |a1|-|a2| segment can pass through the corner.
		if ((u1 ^ u2) * (u1 ^ u3) > -EPSILON) return false;
		return true;
	}

	Point Segment::GetIntersection(const Point& a1, const Point& a2) const
	{
		Point p1 = a1 - a2;
		Point p2 = b - e;
		Point tmp = p1 * (b ^ e) - p2 * (a1 ^ a2);
		float k = p2 ^ p1;
		if (FloatZero(k)) {
			// Lines are parallel, return midpoint as fallback
			return Point((b.x + e.x) * 0.5f, (b.y + e.y) * 0.5f);
		}
